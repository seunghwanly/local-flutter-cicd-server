...
Do not commit real secrets. Use `.env` locally and keep `env.template` or `env.sample` sanitized. `FLUTTER_VERSION` is only a fallback; prefer project-pinned SDKs from `.fvmrc` or `.tool-versions`. If a pulled branch changes Flutter SDK versions, the Python orchestrator must run `fvm flutter precache --ios` before the build continues.
